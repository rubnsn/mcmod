    ...
    public void onUpdate()
    {
        ...
        if (++workingTime > MAX_WORKING_TIME)
        {
            ...
            /*if(entity instanceof TileEntityChest){
            	int size=((TileEntityChest) entity).getSizeInventory();
            	ArrayList<ItemStack> list=new ArrayList();
            	for(int i=0;i<size;i++){
            		ItemStack is=((TileEntityChest) entity).getStackInSlot(i);
            		if(is!=null){
            			if(is.getItem().itemID == Item.wheat.itemID){
            				((TileEntityChest) entity).decrStackSize(i, 1);
            				list.add(new ItemStack(mod_Bamboo.wheatMaterial,1,0));
            				list.add(new ItemStack(mod_Bamboo.wheatMaterial,1,1));
            				break;
            			}else if(is.getItem() instanceof ItemBlock){
            				if(((ItemBlock)is.getItem()).getBlockID()==Block.cobblestone.blockID){
            					((TileEntityChest) entity).decrStackSize(i, 1);
            					list.add(new ItemStack(Block.gravel,1,0));
            					break;
            				}else if(((ItemBlock)is.getItem()).getBlockID()==Block.gravel.blockID){
            					((TileEntityChest) entity).decrStackSize(i, 1);
            					list.add(new ItemStack(Block.sand,1,0));
            					break;
            				}
            			}
            		}
