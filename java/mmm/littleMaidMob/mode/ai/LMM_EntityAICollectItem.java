...
public class LMM_EntityAICollectItem extends EntityAIBase {
    ...
    public LMM_EntityAICollectItem(EntityLittleMaidBase pEntityLittleMaid, float pmoveSpeed) {
    ...
    public boolean shouldExecute() {
        if (theMaid.inventory.getFirstEmptyStack() > -1) {
            ...
            if (!llist.isEmpty()) {
                ...
                if (!ei.isDead && ei.onGround && ei.delayBeforeCanPickup <= 0 && !ei.isBurning() && canEntityItemBeSeen(ei) && (ep == null || ep.getDistanceSq(ei.posX + MathHelper.sin(ep.rotationYaw * 0.01745329252F) * 2.0D, ei.posY, ei.posZ - MathHelper.cos(ep.rotationYaw * 0.01745329252F) * 2.0D) > 7.5D)) {
                    ItemStack lstack = ei.getEntityItem();
                    if ((lstack.getItem() != Items.sugar)) {
                        if ((theMaid.modeController.getActiveModeClass() == null)) {
                            return false;
                        }
                        if ((!theMaid.modeController.getActiveModeClass().checkItemStack(lstack))) {
                            return false;
                        }
                    }
    ...
    public boolean continueExecuting() {
        return !targetItem.isDead && (theMaid.inventory.getFirstEmptyStack() > -1) && theMaid.getDistanceSqToEntity(targetItem) < 100D;
    }
