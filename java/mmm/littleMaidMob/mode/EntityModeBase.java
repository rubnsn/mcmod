...
public abstract class EntityModeBase {
    ...
    public void farrangeBlock() {
        this.getOwner().getNavigator().clearPathEntity();
    }
    ...
    protected boolean canBlockBeSeen(int pX, int pY, int pZ, boolean toTop, boolean do1, boolean do2) {
        ...
        if (movingobjectposition != null && movingobjectposition.typeOfHit == MovingObjectType.BLOCK) {
            // 接触ブロックが指定したものならば
            if (movingobjectposition.blockX == pX && movingobjectposition.blockY == pY && movingobjectposition.blockZ == pZ) {
                return true;
            }
        }
        return false;
    ...
    public EntityLittleMaidBase getOwner() {
        return this.controller.getOwner();
    }
