...
public class MultiModelData implements IExtendedEntityProperties {
    ...
    public void onUpdate() {
        if (owner != null && !isSuplessUpdate) {
            if (isChanged && (--sendCounter <= 0)) {
                MultiModelHandler.Debug("update %s Entity(%d):%s", owner.worldObj.isRemote ? "Client" : "Server", owner.getEntityId(), owner.getClass().toString());
                isChanged = false;
                sendCounter = 0;
                // 情報に変更があった。
    ...
    public void setScale(float pScale) {
        if (owner instanceof EntityAgeable) {
            // 対象がprotectなので無理やり
            try {
                MultiModelHandler.Debug("popSizeB:%f, %f - %f, %f, (%f, %f, %f - %f %f, %f), %s - %s", owner.width, owner.height, owner.yOffset, owner.ySize, owner.boundingBox.minX, owner.boundingBox.minY, owner.boundingBox.minZ, owner.boundingBox.maxX, owner.boundingBox.maxY, owner.boundingBox.maxZ, owner.myEntitySize.toString(), owner.worldObj.isRemote ? "client" : "server");
                ...
                MultiModelHandler.Debug("popSizeA:%f, %f - %f, %f, (%f, %f, %f - %f %f, %f), %s - %s", owner.width, owner.height, owner.yOffset, owner.ySize, owner.boundingBox.minX, owner.boundingBox.minY, owner.boundingBox.minZ, owner.boundingBox.maxX, owner.boundingBox.maxY, owner.boundingBox.maxZ, owner.myEntitySize.toString(), owner.worldObj.isRemote ? "client" : "server");
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
